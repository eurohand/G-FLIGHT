...
const IMG_SRC =
  "https://lh3.googleusercontent.com/Zupg6_TMI0uRGRYQEVJ9ULDslHDG4TSIf1fmQ4zeIoL-cUbMe_lOoYjJoih5LjIARw";
...
function drawScore() {
  ...
  ctx.fillStyle = "Gray";
  ctx.fillText(
    "W: move up, A: move left, S: move down, D: move right, click: shoot, space: ult, esc: home",
    15,
    15
  );
  ...
  for (let i = 0; i < portions.length; i++) {
    if (portions[i].prim) {
      for (let j = 0; j < portions.length; j++) {
        if (
          getDistance(portions[i], portions[j]) < minDistance &&
          portions[j].prim == false
        ) {
          minStartIndex = i;
          minEndIndex = j;
          minDistance = getDistance(
            portions[minStartIndex],
            portions[minEndIndex]
          );
        }
      }
    }
  ...
  enemies.forEach((enemy) => {
    if (
      (left < enemy.x + enemy.w / 2 &&
        ...
        enemy.species === 0) ||
      (Math.sqrt(
        (enemy.x - x) * (enemy.x - x) + (enemy.y - y) * (enemy.y - y)
      ) < ENEMY_SIZE &&
        enemy.species === 1)
    ) {
      this.hp -= 100;
...
Ship.prototype.normalShot = function () {
  shipShoots.push(
    new Shoot(this.x, this.y, 50 * this.cos, -50 * this.sin, true)
  );
};
  ...
  for (let i = 0; i < 15; i++) {
    shipShoots.push(
      new Shoot(
        this.x,
        this.y,
        50 * Math.cos((Math.PI / 180) * i * 24),
        -50 * Math.sin((Math.PI / 180) * i * 24),
        true
      )
    );
  }
  ...
  for (let i = 0; i < 120; i++) {
    shipShoots.push(
      new Shoot(
        this.x,
        this.y,
        50 * Math.cos((Math.PI / 180) * i * 3),
        -50 * Math.sin((Math.PI / 180) * i * 3),
        true
      )
    );
  }
...
Enemy.prototype.shoot = function () {
  if (this.species === 0) {
    shipShoots.push(
      new Shoot(this.x, this.y, this.xSpeed * 5, this.ySpeed * 5, false)
    );
  } else if (this.species === 1) {
    shipShoots.push(
      new Shoot(
        this.x + this.xSpeed * 15,
        this.y + this.ySpeed * 15,
        5,
        5,
        false
      )
  ...
  shipShoots.forEach((shoot) => {
    if (
      (shoot.x >= left &&
        ...
        species === 0) ||
      (Math.sqrt(
        (shoot.x - x) * (shoot.x - x) + (shoot.y - y) * (shoot.y - y)
      ) < ENEMY_SIZE &&
        shoot.my)
    ) {
      this.use = false;
...
for (let i = 0; i < 250; i++) {
  dots.push(
    new Dot(random(width), random(height), random(4), random(14).toString(16))
  );
}
let storeShips = [];
...
for (let i = 0; i < 8; i++) {
  storeShips.push(
    new Ship(
      storeRects[i].x + storeRects[i].w / 2,
      storeRects[i].y + storeRects[i].h * 0.8
    )
  );
  if (i % 2 == 0) {
...
BODY.addEventListener("mousemove", function (event) {
  let bit = Math.sqrt(
    (event.pageX - ship.x) * (event.pageX - ship.x) +
      (event.pageY - ship.y) * (event.pageY - ship.y)
  );
...
function readyMove(direction) {
  if (
    direction === "arrowDown" &&
    (arrow.y === START_Y || arrow.y === STORE_Y)
  ) {
    arrow.y += 75;
  }
...
function gameStore() {
  ...
  for (let i = 0; i < shipShoots.length; i++) {
    shipShoots[i].move();
    if (
      shipShoots[i].y > storeRects[storeSelect].y &&
...
function storeNameText() {
  ...
  fontText(
    30,
    "DOUBLE SHOT",
    storeRects[0].x + storeRects[0].w / 2,
    storeRects[0].y + 20
  );
  fontText(
    30,
  ...
  fontText(
    30,
    "LINE SKILL",
    storeRects[1].x + storeRects[1].w / 2,
    storeRects[1].y + 20
  );
  fontText(
    30,
  ...
  fontText(
    30,
    "WAVE SHOT",
    storeRects[2].x + storeRects[2].w / 2,
    storeRects[2].y + 20
  );
  fontText(
    30,
  ...
  fontText(
    30,
    "CIRCLE SKILL",
    storeRects[3].x + storeRects[3].w / 2,
    storeRects[3].y + 20
  );
  fontText(
    30,
  ...
  fontText(
    30,
    "TRIPLE SHOT",
    storeRects[4].x + storeRects[4].w / 2,
    storeRects[4].y + 20
  );
  fontText(
    30,
  ...
  fontText(
    30,
    "RAIN SKILL",
    storeRects[5].x + storeRects[5].w / 2,
    storeRects[5].y + 20
  );
  fontText(
    30,
  ...
  fontText(
    30,
    "SPREAD SHOT",
    storeRects[6].x + storeRects[6].w / 2,
    storeRects[6].y + 20
  );
  fontText(
    30,
  ...
  fontText(
    30,
    "MINE SKILL",
    storeRects[7].x + storeRects[7].w / 2,
    storeRects[7].y + 20
  );
  fontText(
    30,
